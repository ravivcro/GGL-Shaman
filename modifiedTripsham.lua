...
Action[ACTION_CONST_SHAMAN_RESTORATION] = {
    ...
    TidalWaves						= Action.Create({ Type = "Spell", ID = 53390, Hidden = true		}),	
    TidalWavesBuff					= Action.Create({ Type = "Spell", ID = 53390, Hidden = true		}),		
...
A[3] = function(icon, isMulti)
	...
	local function DamageRotation(unitID)
        ...
        if A.Berserking:IsReady(unitID) and BurstIsON(unitID) and Unit(unitID):IsBoss() then
	...
	local unitGUID = UnitGUID(unitID)	
		...
		--Riptide
		if A.Riptide:IsReady(unitID) and A.LastPlayerCastID ~= A.Riptide.ID and Unit(unitID):HealthPercent() <= 95 and Unit(unitID):HasBuffs(A.Riptide.ID, true) < 1 then
			return A.Riptide:Show(icon)
		end
		
		if A.Riptide:IsReady(unitID) and A.LastPlayerCastID ~= A.Riptide.ID and Unit(unitID):IsTanking() and Unit(unitID):HasBuffs(A.Riptide.ID, true) < 1 then
			return A.Riptide:Show(icon)
		end
